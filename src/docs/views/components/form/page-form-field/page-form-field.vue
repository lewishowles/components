	...
	<component-page>
		...
		<template #introduction>
			<p>
				...
				more, providing compatibility with <code>form-wrapper</code>, allowing a form to be built up
				quickly and simply.
			</p>
		</template>
		...
		<component-props>
			...
			<component-prop id="prop-name">
				...
				<p>
					...
					<code>form-wrapper</code> component, where it is used as the key for the form's data
					collection. As such, its uniqueness will be verified by <code>form-wrapper</code> when
					used together.
				</p>
			</component-prop>
			...
			<component-prop id="prop-validation">
				...
				<p>
					...
					<code>validate</code> function, as well as applying attributes to the field as necessary,
					such as <code>required</code>.
				</p>

				<p>
					Each entry in validation requires at least a <code>rule</code>, outlining the type of
					validation, and a <code>message</code>, which is used if validation fails. Available rules
				...
				<p>
					Requires a value to be set. Adds the <code>required</code> attribute to the field
					automatically.
				</p>
				...
				<p>
					Perform a minimal check to see if the value contains an <code>@</code> symbol. More
					complex verification isn't really necessary, and the only true way to test an email
					address is through verification.
				</p>
				...
				<p>
					Ensure that the provided value has at least size <code>size</code>. For strings, the
					number of characters is used, for arrays, the length of the array, for objects, the number
					of properties, for numbers, the number itself is used, and for numeric strings the integer
					value of the string is used.
				</p>
				...
				<p>
					Ensure that the provided value has at least size <code>min</code>. Values are evaluated as
					in the <code>size</code> rule.
				</p>
				...
				<p>
					Ensure that the provided value is has at most size <code>max</code>. Values are evaluated
					as in the <code>size</code> rule.
				</p>
				...
				<p>
					Ensure that the provided value is has between <code>min</code> and <code>max</code> size.
					Values are evaluated as in the <code>size</code> rule.
				</p>
				...
				<p>
					...
					required depending on that field, such as <code>options</code> for
		...
		<component-slots>
			<component-slot id="slot-default">
				...
				<p>
					Passed through to the <code>default</code> slot of the relevant form field, the
					<code>default</code> slot generally contains the label for the form element.
				</p>
			</component-slot>
		</component-slots>
		...
		<component-events>
			<component-event id="event-v-model">
				...
				<p>
					The current value of the underlying form field will be available via <code>v-model</code>.
				</p>
			</component-event>
		</component-events>
