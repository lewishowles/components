<template>
	<a ref="anchorElement" class="inline-block" v-bind="{ href, ...attributes }" data-test="link-tag">
		<component :is="iconStart" v-if="haveIconStart" class="inline-block stroke-current" :class="{ 'size-text': iconOnly && !showExternalIcon, 'size-[0.857em] me-1': !iconOnly || showExternalIcon }" data-test="link-tag-icon-start" />
