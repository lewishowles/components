	...
	>
		...
		>
			<template v-if="!readOnly">
				<div v-for="option in ratingOptions" :key="option.id" data-part="option">
					<input
						ref="inputReferences"
						v-model="model"
						type="radio"
						class="peer sr-only"
						v-bind="{ id: option.id, value: option.value, name: inputId }"
					...
					>
						...
						<base-icon
							...
							:class="{
								...
								'fill-red-600 text-red-600 hover:fill-red-800 hover:text-red-800': ratingIsSelected(
									option.value,
								),
...
<script setup>
/**
 * A star rating control, styled as a fieldset of radio inputs presented as
 * stars. Built directly on `field-wrapper`/`form-label`/`form-supplementary`
 * rather than `form-radio-group`, since it needs full control over the
 * per-option markup (hidden native input, custom icon, read-only single-icon
 * display) that a generic options loop can't provide.
 */
...
// Shared field boilerplate: ID generation, describedby, and supplementary state.
const { inputId, errorId, describedBy, haveError, haveIntroduction } = useFormField({
	id: props.id,
});
...
const ratingOptions = computed(() =>
	range(1, props.max).map((value) => ({ id: `${inputId.value}-${value}`, value })),
);
