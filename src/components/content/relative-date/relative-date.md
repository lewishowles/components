...
Whether to capitalise dates generated by the relative-time formatter. Disable this to preserve the formatter's original capitalisation.
...
Replaces the `Just now` label when the relative value is zero. The slot receives `value`, `unit`, and `capitalise`.
...
### Preserve formatter capitalisation

```html
<relative-date
	v-bind="{
		capitalise: false,
		date: '2025-03-28T13:15:20',
