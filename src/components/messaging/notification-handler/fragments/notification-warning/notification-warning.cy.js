...
const mount = createMount(NotificationWarning, { props: defaultProps });

describe("notification-warning", () => {
	it("A component is rendered", () => {
		mount();

		cy.getByData("notification-warning").shouldBeVisible();
		cy.getByData("notification-warning").shouldHaveText(notificationMessage);
		cy.getByData("notification-warning-stripe").shouldBeVisible();
		cy.getByData("notification-warning-badge").shouldBeVisible();
	});
	...
	describe("Features", () => {
		it("A simple notification does not include extras", () => {
			...
			cy.getByData("notification-warning-title").should("not.exist");
		});
		...
		describe("Title", () => {
			...
			it("A notification can have a title", () => {
				...
				cy.getByData("notification-warning-title").shouldBeVisible();
			});
		});
		...
		describe("Date", () => {
			it("A date is not shown when it isn't present", () => {
				...
				cy.getByData("notification-warning-date").should("not.exist");
			});

			it("A notification can have a date", () => {
				mount({ notification: { ...notification, date: "2025-03-29" } });

				cy.getByData("notification-warning-date").shouldBeVisible().shouldHaveText("29/03/2025");
			});
			...
			it("The date format can be customised", () => {
				...
				cy.getByData("notification-warning-date").shouldBeVisible().shouldHaveText("29 Mar 2025");
			});
			...
			it("The date locale can be changed", () => {
				...
				cy.getByData("notification-warning-date").shouldBeVisible().shouldHaveText("29.3.2025");
			});
		});
		...
		describe("Icon", () => {
			it("An icon is not shown when it isn't present", () => {
				...
				cy.getByData("notification-warning-icon").should("not.exist");
			});

			it("A notification can have an icon", () => {
				mount({ notification: { ...notification, icon: "icon-user" } });

				cy.getByData("notification-warning-icon").shouldBeVisible();
			});
		});
		...
		describe("Image", () => {
			it("An image is not shown when it isn't present", () => {
				...
				cy.getByData("notification-warning-image").should("not.exist");
			});

			it("A notification can have an image", () => {
				mount({ notification: { ...notification, image_url: "https://placehold.co/100x100" } });

				cy.getByData("notification-warning-image").shouldBeVisible();
			});
		...
		describe("View more", () => {
			it("A link to view more is not shown when no URL is present", () => {
				...
				cy.getByData("notification-warning-view-more").should("not.exist");
			});

			it("A notification can have a link to view more information", () => {
				mount({ notification: { ...notification, url: "https://example.com" } });

				cy.getByData("notification-warning-view-more").shouldBeVisible();
			});
			...
			it("The `view-more-label` slot can be implemented", () => {
				...
				cy.getByData("notification-warning-view-more").shouldHaveText("View something");
			});
		});
	});
});
