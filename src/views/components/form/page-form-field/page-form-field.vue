	...
	<component-page>
		...
		<component-props>
			...
			<component-prop id="prop-name">
				...
				<p>The name of the field. This is required when used within a <code>form-wrapper</code> component, where it is used as the key for the form's data collection. As such, its uniqueness will be verified by <code>form-wrapper</code> when used together.</p>
			</component-prop>
			...
			<component-prop id="prop-validation">
				...
				<p>Any validation to apply to the field. This is used with the externally-facing <code>validate</code> function, as well as applying attributes to the field as necessary, such as <code>required</code>.</p>

				<p>Each entry in validation requires at least a <code>rule</code>, outlining the type of validation, and a <code>message</code>, which is used if validation fails. Available rules and properties include:</p>

				<h4 id="validation-required">
					required
				</h4>

				<p>Requires a value to be set. Adds the <code>required</code> attribute to the field automatically.</p>
				...
				<p>Perform a minimal check to see if the value contains an <code>@</code> symbol. More complex verification isn't really necessary, and the only true way to test an email address is through verification.</p>
				...
				<p>Ensure that the provided value has at least size <code>size</code>. For strings, the number of characters is used, for arrays, the length of the array, for objects, the number of properties, for numbers, the number itself is used, and for numeric strings the integer value of the string is used.</p>
				...
				<p>Ensure that the provided value has at least size <code>min</code>. Values are evaluated as in the <code>size</code> rule.</p>
				...
				<p>Additional props are passed through to the underlying form field. Additional props may be required depending on that field, such as <code>options</code> for <code>radio-group</code>.</p>
			</component-prop>
		</component-props>
