...
- Fixes a clash between Temporal-polyfill versions
...
A new `extendComponent` utility has been added. This allows intercepting a component before using it in a project. For example, you can provide different defaults, or global slot content, such as translations.
...
- `progress-bar`
  - A new `variant` prop allows using as either a progress bar or a `meter`.
  - Now allows passing `null` as the `current` value to show an indeterminate progress bar.
- `data-table`
  - New `search-introduction` and `search-help` slots allow passing introduction and help content to the search field.
